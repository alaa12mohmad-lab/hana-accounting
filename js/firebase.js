...
async function _loadUsers(){
  ...
  if(!el) return;
  try{
    const snap=await firebase.firestore().collection('_users').get();
    const users=snap.docs.map(d=>({uid:d.id,...d.data()}));
    el.innerHTML='<div>'+
      '<div class="page-header"><div class="section-title" style="margin:0">👥 إدارة المستخدمين وصلاحياتهم</div></div>'+
      '<div class="alert alert-blue mb12" style="font-size:11px"><span>ℹ️</span>'+
      '<div><strong>المدير</strong>: كل الصلاحيات &nbsp;|&nbsp; <strong>المحاسب</strong>: إدخال وتعديل وحذف &nbsp;|&nbsp; <strong>المراجع</strong>: قراءة وطباعة فقط</div></div>'+
      '<div class="tbl-wrap"><table><thead><tr><th>البريد الإلكتروني</th><th>الدور الحالي</th><th>آخر تحديث</th><th>تغيير الدور</th></tr></thead>'+
      '<tbody>'+
      (users.length===0?'<tr><td colspan="4" class="tbl-empty"><span class="tbl-empty-icon">👥</span>لا يوجد مستخدمون حتى الآن</td></tr>':
        users.map(u=>{
          const role=u.role||'reviewer';
          const rInfo=ROLES[role]||ROLES.reviewer;
          const isAdminUser=u.email?.toLowerCase()===ADMIN_EMAIL;
          return '<tr>'+
            '<td><strong>'+(u.email||u.uid)+'</strong>'+(isAdminUser?' <span class="badge badge-purple">مدير النظام</span>':'')+'</td>'+
            '<td><span class="badge" style="background:'+rInfo.color+'20;color:'+rInfo.color+';border:1px solid '+rInfo.color+'40;padding:3px 10px;border-radius:20px;font-size:11px;font-weight:700">'+rInfo.label+'</span></td>'+
            '<td class="text-xs text-gray">'+(u.updatedAt?fmtDate(u.updatedAt):'—')+'</td>'+
            '<td>'+(isAdminUser?'<span class="text-xs text-gray">مدير دائم</span>':
              '<div class="flex" style="gap:4px">'+
              Object.entries(ROLES).map(([k,v])=>
                '<button class="btn btn-xs" style="background:'+(role===k?v.color:'#f1f5f9')+';color:'+(role===k?'#fff':'#374151')+';font-size:10px;padding:3px 8px;border-radius:6px;border:none;cursor:pointer;font-family:inherit" onclick="changeUserRole(\''+u.uid+'\',\''+k+'\')">'+v.label+'</button>'
              ).join('')+'</div>')+
            '</td></tr>';
        }).join(''))+
      '</tbody></table></div></div>';
